...
public class DebugBridgeMod extends AbstractDebugBridgeMod implements ClientModInitializer {
    private static final String MC_VERSION = "26.1";
    ...
    protected FabricNamespaceLookup createNamespaceLookup() {
        // Exact 26.1 ships unobfuscated, so the kernel falls back to
    ...
    protected SessionControlProvider createSessionControlProvider() {
        return new Minecraft261SessionControlProvider();
    }
    ...
    private static class Minecraft261StateProvider implements GameStateProvider {
        ...
        public SnapshotDto captureSnapshot() {
            Minecraft mc = Minecraft.getInstance();
            if (mc.isSameThread()) {
                return collectSnapshot(mc);
            }

            CompletableFuture<SnapshotDto> future = new CompletableFuture<>();
            mc.execute(() -> {
                try {
                    future.complete(collectSnapshot(mc));
                } catch (Exception e) {
            ...
            try {
                ...
                throw new RuntimeException("Interrupted while capturing 26.1 snapshot", e);
            } catch (Exception e) {
                throw new RuntimeException("Timed out or failed while capturing 26.1 snapshot", e);
            }
        }

        private SnapshotDto collectSnapshot(Minecraft mc) {
