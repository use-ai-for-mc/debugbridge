...
public class DebugBridgeMod extends AbstractDebugBridgeMod implements ClientModInitializer {
    ...
    private static Component playerMessage(String message, int color) {
        ...
        for (TextLinks.Segment seg : TextLinks.split(message)) {
            if (seg.isLink()) {
                ClickEvent open;
                try {
                    open = new ClickEvent(ClickEvent.Action.OPEN_URL, seg.text());
    ...
    protected void onPostTick() {
        // Block glow was introduced through a different renderer hook after
        // 1.21.4. Keep the endpoint harmless until that hook is ported.
    }

    /**
     * Captures game state for the snapshot endpoint.
     */
    private static class Minecraft1214StateProvider implements GameStateProvider {
        @Override
